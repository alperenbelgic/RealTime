...
export class ChannelService {
    ...
    constructor(
        ...
        this.hubConnection = new signalR.HubConnection('/TheHub');

        //this.hubConnection.url = router.url  ;
        ...
        this.hubConnection.stateChanged((state: any) => {
            ...
            switch (state.newState) {
                case this.hubConnection.connectionState.connecting:
                    newState = ConnectionState.Connecting;
                    break;
                case this.hubConnection.connectionState.connected:
                    newState = ConnectionState.Connected;
                    break;
                case this.hubConnection.connectionState.reconnecting:
                    newState = ConnectionState.Reconnecting;
                    break;
                case this.hubConnection.disconnected:
    ...
    subscribe(methodName: string): Observable<any> {

        var subject = new Subject<any>();

        this.hubConnection.on(methodName, function (data: any) {
            subject.next(data);
        });
